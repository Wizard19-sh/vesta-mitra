...
export default function BetaRunnerPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    const response = await fetch("/api/admin/beta", { method: "POST", headers: { "content-type": "application/json", "x-beta-admin-key": key }, body: JSON.stringify({ recipientId, agent, scenario, confirmation }) });
    const result = await response.json();
    if (result.preview) setPreview(result.preview);
    if (response.ok && result.runKey) setResult(result);
    setNotice(response.ok ? "Preview ready. SEND remains a deliberate live gate." : (result.error ?? "Unable to prepare run"));
  }

  return <main className={styles.shell}><header className={styles.header}><Link className={styles.back} href="/admin/runs">← Runs</Link><div><p>Internal only</p><h1>Beta runner</h1></div><span>One recipient at a time</span></header><section className={styles.trace}><p className={styles.kicker}>Meta development beta</p><h2>Prepare one live run</h2><p>{notice}</p>{!recipients.length ? <div className={styles.section}><label>Internal beta key<input type="password" value={key} onChange={(event) => setKey(event.target.value)} autoComplete="off" /></label><button type="button" onClick={load}>Load recipients</button></div> : <form className={styles.section} onSubmit={submit}><label>Recipient<select value={recipientId} onChange={(event) => setRecipientId(event.target.value)}>{recipients.map((recipient) => <option key={recipient.id} value={recipient.id} disabled={!recipient.enabled}>{recipient.displayName} — {recipient.maskedPhone}{recipient.enabled ? "" : " (disabled)"}</option>)}</select></label>{selected && <p><strong>Role:</strong> {selected.role}{selected.label ? ` · ${selected.label}` : ""}</p>}<label>Agent<select value={agent} onChange={(event) => setAgent(event.target.value as "mitra" | "tarla")}><option value="mitra">Mitra</option><option value="tarla">Tarla</option></select></label><p><strong>Scenario:</strong> {agent === "mitra" ? "Evening walk reminder" : "Meal plan with Palak nahi hai exception"}</p><label>Type SEND to confirm<input value={confirmation} onChange={(event) => setConfirmation(event.target.value)} autoComplete="off" /></label><button type="submit">Preview / confirm</button>{preview && <pre className={styles.section}>{preview}</pre>}{result && <section className={styles.section}><h3>Run result</h3><p><strong>Run:</strong> {result.runId ?? "not yet available"}</p><p><strong>Evidence:</strong> {result.evidenceId ?? "pending a real reply"}</p><p><strong>Provider:</strong> {result.providerStatus ?? "not reported"}</p><p><strong>Provider message:</strong> {result.providerMessageId ?? "not reported"}</p></section>}</form>}</section></main>;
}
