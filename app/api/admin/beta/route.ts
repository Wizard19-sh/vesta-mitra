...
import { executeProvenW4 } from "../../../../lib/betaW4Execution";
...
export async function POST(request: NextRequest) {
  if (!authorised(request)) return denied();
  try {
    const body = await request.json() as { recipientId?: string; agent?: string; scenario?: string; confirmation?: string };
    ...
    const preview = body.agent === "mitra"
      ? composeMitraMessage({ recipientSalutation: recipient.displayName, seniorSalutation: recipient.displayName, label: "evening walk", type: "Walk / activity", language: "Hinglish", context: { agent: "mitra", audience: "senior", surface: "whatsapp", moment: "reminder" } })
      : "Tarla will generate this recipient's current approved plan when SEND is confirmed.";
    if (body.confirmation !== "SEND") return NextResponse.json({ recipient: recipientView(recipient), preview, sendAllowed: false });
    const result = await executeProvenW4({ recipient, agent: body.agent });
    return NextResponse.json({
