...
function OnboardingFlow({ ownerKey, existingSession }: { ownerKey: string; existingSession: ExistingSession | null }) {
  ...
  const [mitraAddedMemberKey, setMitraAddedMemberKey] = useState<string>();
  ...
  async function submitIdentity(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
      const mobile = splitPhone(identity.mobilePhone);
      const result = await createIdentity({
        ownerKey,
        name: identity.name,
        email: identity.email || undefined,
        mobilePhone: normalizePhone(mobile.countryCode, mobile.localNumber),
      ...
      setMembers((current) => {
        ...
        const savedPrimary = defaultHouseholdMember({
          ...
          whatsappNumber: identity.mobilePhone,
          isPrimary: true,
        });
  ...
  function submitTarlaFood(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!tarla.mealSlots.length) return setError("Choose at least one meal for Tarla to plan.");
  ...
  function submitTarlaCooks(event: FormEvent<HTMLFormElement>) {
    ...
    for (const cook of tarla.cookingPeople) {
      ...
      if (!validPhoneDraft(cook.phone || member.whatsappNumber || "")) return setError(`Add a valid WhatsApp number for ${member.name}.`);
  ...
  return (
    <main className={styles.shell}>
      <header className={styles.header}>
        <div className={styles.brandLockup}><AeviaLogo compact /><span><i />Domestic setup</span></div>
        ...
        <Panel eyebrow="About you" title="Let's start with you." supporting="A few basics so Aevia knows whose household it's helping.">
          <form className={styles.form} onSubmit={submitIdentity}>
            <div className={styles.twoColumns}>
              ...
              <PhoneField label="Mobile number" value={identity.mobilePhone} onChange={(mobilePhone) => setIdentity({ ...identity, mobilePhone })} />
            </div>
            <Field label="Email" hint="Optional"><input type="email" value={identity.email} onChange={(e) => setIdentity({ ...identity, email: e.target.value })} placeholder="you@example.com" /></Field>
            ...
            <FormError error={error} /><NextButton busy={busy}>Accept and continue</NextButton>
          </form>
        </Panel>
      )}
        ...
        <Panel eyebrow="Phase 04 · Mitra 1 of 2 — Care Specialist" title="Who should Mitra help?" supporting="Choose from the people already in your household. You can select more than one.">
          <form className={styles.form} onSubmit={submitMitraWho}>
            <div className={styles.selectPeople}>
              {householdMembers.filter((member) => member.lifeStage !== "child").map((member) => {
            ...
            <button className={styles.addButton} type="button" onClick={() => { const member = defaultHouseholdMember(); setMembers((current) => [...current, member]); setMitraAddedMemberKey(member.clientKey); }}>+ Add another person</button>
            {mitraAddedMemberKey && members.some((member) => member.clientKey === mitraAddedMemberKey) && <MemberEditor member={memberByKey(members, mitraAddedMemberKey)} onChange={(next) => updateMemberState(setMembers, next)} onRemove={() => { removeMember(memberByKey(members, mitraAddedMemberKey)); setMitraAddedMemberKey(undefined); }} />}
            {mitraPeople.map((person) => {
              const member = memberByKey(members, person.memberClientKey);
              return (
                <section className={styles.configCard} key={person.memberClientKey}>
                  <h2>{member.name}</h2>
                  <div className={styles.reusedDetails}><span>Uses shared household details</span><strong>{member.preferredSalutation || member.name} · {member.preferredLanguage}</strong><button type="button" onClick={() => setStep("household")}>Edit household details</button></div>
                  ...
                  {(person.communicationPath === "senior_directly" || person.communicationPath === "both") && (member.whatsappNumber ? <p className={styles.savedDetail}>WhatsApp: saved from the shared member record</p> : <PhoneField label={`${member.preferredSalutation || member.name}'s WhatsApp`} value={person.directPhone} onChange={(value) => { updateMemberState(setMembers, { ...member, whatsappNumber: value }); patchMitraPerson(person.memberClientKey, { directPhone: value }); }} />)}
      ...
      {step === "mitraRoutines" && (
        <Panel eyebrow="Phase 04 · Mitra 2 of 2 — Routine Protocols" title="What should Mitra help with?" supporting="Choose one or more routines. Add as many agreed routines as this person needs.">
          <form className={styles.form} onSubmit={submitMitraRoutines}>
            {mitraPeople.map((person) => {
              const member = memberByKey(members, person.memberClientKey);
              return <section className={styles.routineGroup} key={person.memberClientKey}><div className={styles.groupTitle}><div><p>For {member.preferredSalutation || member.name}</p><span>{pathLabel(person.communicationPath)}</span></div><div><button type="button" onClick={() => patchMitraPerson(person.memberClientKey, { routines: [...person.routines, defaultRoutine()] })}>+ Add another routine</button><button type="button" onClick={() => patchMitraPerson(person.memberClientKey, { routines: [...person.routines, { ...defaultRoutine(), type: "Medication", label: "" }] })}>+ Add another medicine</button></div></div>{person.routines.map((routine, index) => <RoutineEditor key={routine.clientKey} routine={routine} index={index} member={member} path={person.communicationPath} caretaker={person.caretakerMemberClientKey ? memberByKey(members, person.caretakerMemberClientKey) : undefined} onChange={(next) => patchRoutine(person.memberClientKey, next)} onRemove={() => patchMitraPerson(person.memberClientKey, { routines: person.routines.filter((item) => item.clientKey !== routine.clientKey) })} />)}</section>;
            })}
      ...
      {step === "tarlaFood" && (
        <Panel eyebrow="Phase 04 · Tarla 2 of 4 — Food Preferences" title="What does your household actually like eating?" supporting="This helps Tarla suggest meals that feel familiar, not generic.">
          <form className={styles.form} onSubmit={submitTarlaFood}>
            <Field label="Which meals should Tarla plan?"><Pills values={["breakfast", "lunch", "snack", "dinner"]} labels={["Breakfast", "Lunch", "Snack", "Dinner"]} selected={tarla.mealSlots} onToggle={(value) => setTarla({ ...tarla, mealSlots: toggleText(tarla.mealSlots, value) })} /></Field>
            <Field label="Dietary pattern"><Pills values={["vegetarian", "eggetarian", "non_vegetarian"]} labels={["Vegetarian", "Eggetarian", "Non-vegetarian"]} selected={[tarla.dietaryType]} single onToggle={(value) => setTarla({ ...tarla, dietaryType: value as TarlaSetupDraft["dietaryType"] })} /></Field>
      ...
      {step === "tarlaCooks" && (
        <Panel eyebrow="Phase 04 · Tarla 4 of 4 — Cooking Coordination" title="Who prepares meals?" supporting="Tarla adapts how it coordinates based on who cooks and when they visit.">
          <form className={styles.form} onSubmit={submitTarlaCooks}>
            <div className={styles.cookChoice}><button type="button" onClick={() => addCookingPerson("hired_cook")}>Hired cook</button><button type="button" onClick={() => addCookingPerson("family_cook")}>Family member</button><button type="button" onClick={() => addCookingPerson("primary_user")}>I cook</button><button type="button" onClick={() => addCookingPerson("other")}>Different people</button></div>
      ...
      <footer className={styles.onboardingFooter}><span>© 2026 Aevia · Closed beta</span><nav><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link><Link href="/beta">Beta status</Link></nav></footer>
    </main>
  );
  ...
  function toggleMitraPerson(member: HouseholdMemberDraft) {
    ...
    setMitraPeople(existing ? mitraPeople.filter((item) => item.memberClientKey !== member.clientKey) : [...mitraPeople, { memberClientKey: member.clientKey, communicationPath: "senior_directly", directPhone: member.whatsappNumber || "", caretakerPhone: "", consentConfirmed: false, routines: [defaultRoutine()] }]);
  }
  ...
  function addCookingPerson(type: CookingPersonDraft["relationshipType"]) {
    ...
    const knownMember = members.find((item) => item.clientKey === memberKey);
    const cook: CookingPersonDraft = { clientKey: `cook_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`, memberClientKey: memberKey, relationshipType: type, phone: knownMember?.whatsappNumber || (type === "primary_user" ? identity.mobilePhone : ""), preferredLanguage: knownMember?.preferredLanguage ?? "Hinglish", consentConfirmed: type === "primary_user", visits: [defaultVisit(type === "primary_user" ? "Meal reminder" : "Cooking time")] };
...
function NutritionEditor({ member, value, onChange }: { member: HouseholdMemberDraft; value: NutritionPersonDraft; onChange: (value: NutritionPersonDraft) => void }) {
  const height = heightFeetInchesFromCm(value.heightCm);
  function updateHeight(feet: number | undefined, inches: number | undefined) {
    onChange({ ...value, heightCm: feet ? heightCmFromFeetInches(feet, inches ?? 0) : undefined });
  }
  return <details className={styles.nutritionCard} open><summary><span><strong>{member.name}</strong><small>{value.enabled ? "Nutrition goal" : "Balanced"}</small></span></summary><label className={styles.checkLine}><input type="checkbox" checked={value.enabled} onChange={(e) => onChange({ ...value, enabled: e.target.checked })} /><span>Use a nutrition goal for {member.name}</span></label>{value.enabled && <><p className={styles.nutritionWhy}>Tarla uses these details to estimate {member.name}&apos;s needs and plan portions accordingly.</p><div className={styles.twoColumns}><Field label="Age" hint="Years"><input type="number" min={18} max={120} value={value.age ?? ""} onChange={(e) => onChange({ ...value, age: Number(e.target.value) || undefined })} /></Field><Field label="Sex used by the calculation"><select value={value.sex ?? ""} onChange={(e) => onChange({ ...value, sex: e.target.value as "male" | "female" })}><option value="">Choose</option><option value="female">Female</option><option value="male">Male</option></select></Field></div><div className={styles.twoColumns}><Field label="Height" hint="Feet and inches"><div className={styles.heightInputs}><div><input aria-label="Height feet" type="number" min={3} max={8} placeholder="5" value={height.feet ?? ""} onChange={(e) => updateHeight(Number(e.target.value) || undefined, height.inches)} /><span>ft</span></div><div><input aria-label="Height inches" type="number" min={0} max={11} placeholder="8" value={height.inches ?? ""} onChange={(e) => updateHeight(height.feet, Number(e.target.value))} /><span>in</span></div></div></Field><Field label="Weight" hint="Kilograms"><input aria-label="Weight" type="number" min={25} max={300} placeholder="e.g. 70" value={value.weightKg ?? ""} onChange={(e) => onChange({ ...value, weightKg: Number(e.target.value) || undefined })} /></Field></div><Field label="Activity level"><select value={value.activityLevel ?? ""} onChange={(e) => onChange({ ...value, activityLevel: e.target.value as NutritionPersonDraft["activityLevel"] })}><option value="">Choose</option><option value="sedentary">Sedentary · little or no exercise</option><option value="lightly_active">Light · gentle activity 1–2 days/week</option><option value="moderately_active">Moderate · active 3–5 days/week</option><option value="very_active">Very active · exercise 6–7 days/week</option><option value="extra_active">Extra active · intense daily activity</option></select></Field><Field label="Goal"><select value={value.goal === "custom" ? "maintain" : value.goal} onChange={(e) => onChange({ ...value, goal: e.target.value as NutritionPersonDraft["goal"] })}><option value="maintain">Maintain</option><option value="moderate_deficit">Gentle calorie deficit</option><option value="stronger_deficit">Higher calorie deficit</option><option value="high_protein">Higher protein</option></select></Field></>}</details>;
}

function CookEditor({ cook, index, members, onMemberChange, onChange, onRemove }: { cook: CookingPersonDraft; index: number; members: HouseholdMemberDraft[]; onMemberChange: (member: HouseholdMemberDraft) => void; onChange: (cook: CookingPersonDraft) => void; onRemove: () => void }) {
  const member = memberByKey(members, cook.memberClientKey);
  const choosesHouseholdMember = cook.relationshipType === "family_cook" || cook.relationshipType === "primary_user";
  const isPrimary = cook.relationshipType === "primary_user";
  const isExternal = cook.relationshipType === "hired_cook" || cook.relationshipType === "other";
  function patchVisit(clientKey: string, patch: Partial<(typeof cook.visits)[number]>) {
    onChange({ ...cook, visits: cook.visits.map((visit) => visit.clientKey === clientKey ? { ...visit, ...patch } : visit) });
  }
  return <article className={styles.cookCard}>
    <header><div><strong>{isPrimary ? "You cook" : `Cooking person ${index + 1}`}</strong><span>{cookRoleLabel(cook.relationshipType)}</span></div><button type="button" onClick={onRemove}>Remove</button></header>
    {choosesHouseholdMember ? <Field label={isPrimary ? "Cooking person" : "Choose household member"}><select value={cook.memberClientKey} disabled={isPrimary} onChange={(event) => { const nextMember = memberByKey(members, event.target.value); onChange({ ...cook, memberClientKey: nextMember.clientKey, phone: nextMember.whatsappNumber || "", preferredLanguage: nextMember.preferredLanguage }); }}>{members.filter((item) => item.memberKind === "household").map((item) => <option key={item.clientKey} value={item.clientKey}>{item.name}</option>)}</select></Field> : <div className={styles.twoColumns}><Field label="Name"><input value={member.name} onChange={(event) => onMemberChange({ ...member, name: event.target.value, preferredSalutation: member.preferredSalutation || event.target.value })} placeholder="Name" /></Field><Field label="How should Tarla address them?"><input value={member.preferredSalutation} onChange={(event) => onMemberChange({ ...member, preferredSalutation: event.target.value })} placeholder="e.g. Sunita didi" /></Field></div>}
    {isPrimary ? <div className={styles.reusedDetails}><span>Uses your saved details</span><strong>{member.name} · {member.preferredLanguage} · mobile saved</strong></div> : member.whatsappNumber && !isExternal ? <div className={styles.reusedDetails}><span>Uses shared household details</span><strong>{member.preferredSalutation || member.name} · {member.preferredLanguage} · WhatsApp saved</strong></div> : <div className={styles.twoColumns}><PhoneField label="WhatsApp number" value={cook.phone} onChange={(phone) => { onChange({ ...cook, phone }); onMemberChange({ ...member, whatsappNumber: phone }); }} /><Field label="Preferred language"><select value={cook.preferredLanguage} onChange={(event) => { const preferredLanguage = event.target.value as AeviaLanguage; onChange({ ...cook, preferredLanguage }); onMemberChange({ ...member, preferredLanguage }); }}><option>English</option><option>Hindi</option><option>Hinglish</option></select></Field></div>}
    <div className={styles.visitList}>{cook.visits.map((visit, visitIndex) => <section className={styles.visitCard} key={visit.clientKey}><header><strong>{isPrimary ? `Reminder ${visitIndex + 1}` : visit.label}</strong>{cook.visits.length > 1 && <button type="button" onClick={() => onChange({ ...cook, visits: cook.visits.filter((item) => item.clientKey !== visit.clientKey) })}>Remove</button>}</header>{!isPrimary && <Field label="Schedule label"><input value={visit.label} onChange={(event) => patchVisit(visit.clientKey, { label: event.target.value })} placeholder="Morning cooking" /></Field>}<DayPicker selected={visit.daysOfWeek} onChange={(days) => patchVisit(visit.clientKey, { daysOfWeek: days })} /><Time12Field label={isPrimary ? "When should Tarla remind you?" : "Cooking time"} value={visit.time12} onChange={(time12) => patchVisit(visit.clientKey, { time12 })} /><Field label="Meals"><Pills values={["breakfast", "lunch", "snack", "dinner"]} labels={["Breakfast", "Lunch", "Snack", "Dinner"]} selected={visit.mealSlots} onToggle={(value) => patchVisit(visit.clientKey, { mealSlots: toggleText(visit.mealSlots, value) })} /></Field></section>)}</div>
    <button className={styles.textButton} type="button" onClick={() => onChange({ ...cook, visits: [...cook.visits, defaultVisit(isPrimary ? "Meal reminder" : "Cooking time", "6:00 PM")] })}>+ Add another {isPrimary ? "reminder" : "cooking time"}</button>
    {!isPrimary && <label className={styles.checkLine}><input type="checkbox" checked={cook.consentConfirmed} onChange={(event) => onChange({ ...cook, consentConfirmed: event.target.checked })} /><span>They have agreed to receive Tarla’s kitchen messages.</span></label>}
    {!isPrimary && <div className={styles.preview}><span>Introduction preview</span><p>{composeCookIntroduction({ cookName: member.preferredSalutation || member.name || "there", language: cook.preferredLanguage, relationshipType: cook.relationshipType })}</p></div>}
  </article>;
}
...
function initialState(existing: ExistingSession | null) {
  const identity = { name: existing?.profile.name ?? "", email: existing?.profile.email ?? "", mobilePhone: existing?.profile.mobilePhone ?? existing?.member.whatsappNumber ?? "+91", householdName: existing?.household.name ?? "", timezone: existing?.household.timezone ?? (typeof window === "undefined" ? "Asia/Kolkata" : Intl.DateTimeFormat().resolvedOptions().timeZone), accepted: Boolean(existing) };
  ...
  const storedMembers: HouseholdMemberDraft[] = existing.setup.members.map((member) => ({ clientKey: String(member._id), memberId: String(member._id), name: member.name, relationship: member.relationship ?? (member._id === existing.member._id ? "Self" : member.role), lifeStage: storedLifeStage(member), preferredSalutation: member.preferredSalutation ?? (member._id === existing.member._id ? member.name : ""), preferredLanguage: supportedLanguage(member.languagePreference), whatsappNumber: member.whatsappNumber ?? (member._id === existing.member._id ? existing.profile.mobilePhone : "") ?? "", memberKind: member.memberKind ?? (/external|cook|cooking/i.test(member.role) ? "external" : "household"), isPrimary: member._id === existing.member._id }));
  ...
  if (existing.setup.tarla) {
    ...
    tarla.mealSlots = existing.setup.tarla.householdProfile?.mealsPreparedAtHome ?? existing.setup.tarla.eaterProfiles[0]?.profile.mealsAtHome ?? tarla.mealSlots;
    ...
    tarla.softerPreferences = listFromText(existing.setup.tarla.softerPreferences);
...
function defaultVisit(label = "Cooking time", time12 = "8:00 AM") { return { clientKey: `visit_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`, label, daysOfWeek: [...ALL_DAYS], time12, mealSlots: ["breakfast", "lunch", "snack", "dinner"] }; }
...
function nutritionGoalLabel(goal: NutritionPersonDraft["goal"]) { return goal === "maintain" ? "Maintain" : goal === "moderate_deficit" ? "Gentle calorie deficit" : goal === "stronger_deficit" ? "Higher calorie deficit" : goal === "high_protein" ? "Higher protein" : "Maintain"; }
