...
| `EVD-017` | Pari-approved Tarla instruction to Vaibhav: Meta accepted the outbound request, then reported delivery failure; no inbound reply and no retry | Failed-run evidence | Observability | N — failed delivery is preserved honestly and is not a successful real-output claim | Convex development run `066742c8-2d95-4565-b59d-cc15b1223e57`; evidence record `EVD-RUN-066742c8-2d95-4565-b59d-cc15b1223e57`; accepted 2026-09-05 06:20:47.330 IST; failed 2026-09-05 06:20:48.000 IST |
| `EVD-018` | Real Meta “Hi” messages from Mohit and Krishna reached the signed webhook and opened provider conversation windows. Mohit's stored inbound is now linked to his canonical Household 1 member/contact after five historical duplicate endpoints were disabled; Krishna remains unmatched. No outbound message was sent | Inbound readiness evidence | Observability | N — valid real inbound and deterministic contact-resolution proof only; no completed task claim | Mohit provider message `wamid.HBgMOTE5ODMzNjU3NzY0FQIAEhggQUM1NEMwNjAwNkNCQ0FFQzI3QjcxNTJGMjM1MUQ1MUUA` at 2026-09-05 06:30:02 IST; masked recipient `+********7764`; canonical member `k173ygx3rd1fktn1s39r73ah818dnykt`; canonical endpoint `js720fqcdcfdth702ynrzj68gn8dmj7j`; exact no-send preview `Papa, evening walk ka time ho gaya.`; Meta authentication passed; explicit SEND gate remained closed. Krishna provider message `wamid.HBgMOTE5ODIwMzEzNTMzFQIAEhgUM0FDRTA0MDZDNkM1QTA0MURBQ0IA` at 2026-09-05 06:30:28 IST |
| `EVD-019` | Meta template-bootstrap readiness checked without sending: the existing test sender exposes approved `hello_world` (`en_US`, utility), and Mohit remains the sole prepared recipient for this gate | Conversation bootstrap readiness | Configuration evidence | N — proves template availability and authentication only; it is not an Aevia agent output or delivery claim | Failed Mitra run `62af13cc-717a-496f-989d-6181d2772b82` / `EVD-RUN-62af13cc-717a-496f-989d-6181d2772b82` preserved unchanged; masked Mohit `+********7764`; Meta authentication passed; no template or Aevia message sent |
| `EVD-020` | Existing Meta app subscribed to the configured development WABA after error `131005`; subscription verified by matching the authenticated app inside `subscribed_apps` | Meta configuration correction | Configuration evidence | N — readiness only; no delivery or completed-task claim | Before the correction the field-filtered check reported zero; Meta accepted the subscription POST, and the authoritative nested response then showed the authenticated app present among two subscriptions. Mohit remained inside the validated 24-hour window; exact no-send preview `Papa, evening walk ka time ho gaya.`; Meta auth HTTP 200; no WhatsApp send |
| `EVD-021` | Latest Mohit Mitra send failed synchronously before Meta issued a provider message ID | Failed real-output attempt | Transport failure evidence | N — failure evidence only; no delivery or completed-task claim | Run `84a6560b-30d7-4ea9-9103-e66d032a99ca`; evidence `EVD-RUN-84a6560b-30d7-4ea9-9103-e66d032a99ca`; requested 2026-09-05 07:06:13.756 IST; failed 07:06:14.260 IST; stored code `meta_131005`; no `wamid`, accepted timestamp, status webhook, or inbound reply. Read-only checks confirmed valid unexpired USER token, both required WhatsApp permissions targeted to the configured WABA, current app subscription, configured Cloud API sender belonging to the WABA, `CONNECTED` status, green quality, and `NOT_VERIFIED` code-verification state. Full Meta response fields were not persisted, so HTTP status, title, details, and `fbtrace_id` are not claimed |
| `EVD-022` | New live-created Household 1 identified and verified; existing validated Mohit and Krishna inbound records were moved to its canonical shared members/endpoints while older active duplicates were disabled and preserved | Household/contact readiness evidence | Configuration and persistence evidence | N — readiness only; no outbound or completed-task claim | Household `jx7dzy85wenqt7maybvsxnf7nh8dv4em`; Siddharth `k1730t3cxaqdhjgqhkd5dp8g218dt7b7`; Mohit `k175prz6g8va7d3d4xv7ysxvrs8dtdrs` / endpoint `js70mwg0a9ydwxatb3shr51pts8dv3wg`; Krishna `k17cfkm6dkgx8d72xpm5x7n1sx8dtsma` / endpoint `js7363sgr149s0x2dna24kba1s8dvx2z`. Both signed inbound windows remained open and Meta auth passed. User-approved plan `nd72jve025asame64pc07fjffs8dvpaj` version 1 targets 2026-09-06, but its scheduled Krishna execution has no persisted instruction, so exact prepare failed closed. No Ivvan member record exists in this saved household; no record was invented |
| `EVD-023` | Krishna's existing scheduled execution was converted into an exact prepared payload without dispatch | Tarla preparation evidence | Prepared real-household output | N — exact persisted preview only; no provider acceptance or delivery claim | Plan `nd72jve025asame64pc07fjffs8dvpaj` version 1; execution `m17apqn3n1bvp1aq4dwgv0dy2h8dvpav`; run `2a85e653-bdc2-493e-8b78-b4a5128e0c07`. The original future scheduler job was cancelled, the instruction was composed through `composeDayExecutionInstruction`, and both stored instruction fields equal the repeated preview. Zero development or provider transport messages exist. Krishna's validated inbound window remained open. Meta authentication subsequently returned HTTP 401 because the temporary token expired, so SEND remains blocked and no WhatsApp message was sent |
| `EVD-024` | Real Household 1 Tarla instruction was delivered and read by Krishna; her natural reply entered the same signed run but remained unresolved | Partial real WhatsApp loop | Real-output and inbound evidence | N — real delivery and inbound proof only; no autonomous resolution claim | Run `2a85e653-bdc2-493e-8b78-b4a5128e0c07`; evidence reference `EVD-RUN-2a85e653-bdc2-493e-8b78-b4a5128e0c07`; outbound provider ID `wamid.HBgMOTE5ODIwMzEzNTMzFQIAERgSOUE1NkFEQTgwNTlDRDBEOTQ5AA==`, accepted 2026-09-05 08:00:20.230 IST, delivered 08:00:21, read 08:01:27. Raw inbound `Tofu order karna padega. No problem.`; provider ID `wamid.HBgMOTE5ODIwMzEzNTMzFQIAEhgUM0FDMjdBRDk4MEVBMjg4N0Y3MDAA`; message timestamp 08:02:09, webhook received 08:02:10.158 and validated 08:02:10.159. It mapped to canonical Krishna and the existing execution/run. Deterministic interpretation returned unrelated because procurement phrasing plus a mixed acknowledgement is unsupported; no shopping update, response, Sid contact, or primary-user intervention occurred; run remains waiting and execution unresolved |
...
**Next available evidence ID: `EVD-025`.** Add the record when the next qualifying run or artifact is accepted; do not pre-allocate a passing result.
...
- Run `2a85e653-bdc2-493e-8b78-b4a5128e0c07` retains its original recorded interpretation of the real Krishna inbound as unrelated. The historical record was not rewritten. A deterministic mixed-intent correction is covered by non-live regression tests; live confirmation remains pending.
- Mixed-intent deployment: commit `50896cbf9c8324c7d42600f42d73d20d21967b62` is live at `https://helloaevia.vercel.app` with Convex development `dev:grand-goshawk-952`. Root and `/admin/beta` returned HTTP 200, the admin registry exposed five masked recipients with one Krishna entry and no raw-number field, Meta authentication passed with HTTP 200, and Krishna's validated inbound left the 24-hour conversation window open. No WhatsApp message was sent; one fresh real reply is still required to verify the new interpretation and acknowledgement path.
- Krishna mixed-intent learning chain:
  - Real user feedback 1: `Tofu order karna padega. No problem.` → preserved correctly but interpreted as unrelated.
  - Narrow deterministic fix → regression passed and deployed in `50896cbf9c8324c7d42600f42d73d20d21967b62`.
